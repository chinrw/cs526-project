...
z3::expr KintRangeAnalysisPass::ValuetoSymbolicVar(Value *arg) {
  if (auto it = argValuetoSymbolicVar.find(arg);
      it != argValuetoSymbolicVar.end()) {
...
bool KintRangeAnalysisPass::addRangeConstaints(const KintConstantRange &range,
  ...
  Solver.value().add(z3::ule(
      BV, Solver.value().ctx().bv_val(range.getUnsignedMax().getZExtValue(),
                                      range.getBitWidth())));
  Solver.value().add(z3::uge(
      BV, Solver.value().ctx().bv_val(range.getUnsignedMin().getZExtValue(),
                                      range.getBitWidth())));
  return true;
}
...
void KintRangeAnalysisPass::smtSolver(Module &M) {
  ...
  for (auto F : taintedFunctions) {
    ...
    for (auto &arg : F->args()) {
      ...
      const auto argName =
          F->getName().str() + "_" + std::to_string(arg.getArgNo());
      const auto argv = Solver.value().ctx().bv_const(
          argName.c_str(), arg.getType()->getIntegerBitWidth());
...
void KintRangeAnalysisPass::pathSolver(BasicBlock *curBB, BasicBlock *predBB) {
  ...
  if (nullptr != predBB) {
    if (auto terminator = predBB->getTerminator();
      ...
      if (branch->isConditional()) {
        if (auto cmp = dyn_cast<ICmpInst>(branch->getCondition())) {
          ...
          if (!lhs->getType()->isIntegerTy() ||
            ...
            if (impossibleBranches.find(cmp) != impossibleBranches.end() &&
            ...
            const auto check = [cmp, isTrueBranch, this] {
              if (Solver.value().check() == z3::unsat) {
                outs() << "[SMT solving] cannot continue"
                       << (isTrueBranch ? "true" : "false") << " branch of "
                       << *cmp << "\n";
                return false;
              }
              return true;
            ...
            if (isTrueBranch) {
              ...
              argValuetoSymbolicVar[cmp] =
                  Solver.value().ctx().bv_val(false, 1);
            }
          }
        }
      }
      ...
      if (cond->getType()->isIntegerTy()) {
        ...
        auto emptyRange =
            KintConstantRange(cond->getType()->getIntegerBitWidth(), false);
  ...
  for (auto &Inst : *curBB) {
    ...
    if (auto OP = dyn_cast<BinaryOperator>(&Inst)) {
      ...
      if (!addRangeConstaints(getRangeByBB(&Inst, Inst.getParent()),
                              ValuetoSymbolicVar(OP)))
        return;
      ...
      argValuetoSymbolicVar[&Inst] = Solver.value().ctx().bv_const(
          name.c_str(), Inst.getType()->getIntegerBitWidth());
      if (!addRangeConstaints(getRangeByBB(&Inst, Inst.getParent()),
                              ValuetoSymbolicVar(&Inst)))
        return;
    }
  }
